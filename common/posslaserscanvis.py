...
class LaserScanVis:
  ...
  def update_scan(self):
    # first open data
    self.scan.open_scan(self.scan_names[self.offset], self.tag_names[self.offset])
    if self.semantics:
      self.scan.open_label(self.label_names[self.offset], self.tag_names[self.offset])
      self.scan.colorize()

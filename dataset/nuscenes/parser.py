...
class SemanticKitti(Dataset):
    ...
    def __init__(self, root,  # directory where data is
        ...
        self.n_input_scans = sensor["n_input_scans"]
        self.use_residual = sensor["residual"]
        self.transform_mod = sensor["transform"]
        self.use_normal = sensor["use_normal"] if 'use_normal' in sensor.keys(
        ) else False
        """"""
        ...
        for seq in self.sequences:
            ...
            seq = '{0:04d}'.format(int(seq))  # to string
            ...
            if self.use_residual:
                for i in range(self.n_input_scans):
                    ...
                    exec("self.residual_files_" + str(i+1) +
                         "[seq]" + " = " + "residual_files_" + str(i+1))
    ...
    def __getitem__(self, dataset_index):
        ...
        for index in range(start_index, start_index + 1):
            ...
            if self.use_residual:
                for i in range(self.n_input_scans):
                    exec("proj_residuals_" + str(i+1) +
                         " = torch.Tensor(np.load(residual_file_" + str(i+1) + "))")
                    # if flip_sign:
            ...
            proj = (
                proj - self.sensor_img_means[:, None, None]) / self.sensor_img_stds[:, None, None]

            proj_full = torch.cat([proj_full, proj])

        if self.use_normal:
            proj_full = torch.cat([proj_full, torch.from_numpy(
                scan.normal_map).clone().permute(2, 0, 1)])  # 5 + 3 = 8 channel
            # proj_full = torch.cat([proj_full, proj_xyz.clone().permute(2, 0, 1)]) # 5 + 3 = 8 channel

        # add residual channel
        if self.use_residual:
            for i in range(self.n_input_scans):
                proj_full = torch.cat([proj_full, torch.unsqueeze(
    ...
    def remove_few_static_frames(self):
        ...
        remove_mapping_path = os.path.join(os.path.dirname(
            __file__), "../../../config/train_split_dynamic_pointnumber.txt")
        ...
        for seq in self.sequences:
            ...
            if seq in pending_dict.keys():
                ...
                useful_scan_paths = [path for path in scan_files if os.path.split(
                    path)[-1][:-4] in pending_dict[seq]]
                self.scan_files[seq] = useful_scan_paths

                # label_files
                label_files = self.label_files[seq]
                useful_label_paths = [path for path in label_files if os.path.split(
                    path)[-1][:-6] in pending_dict[seq]]
                ...
                if self.use_residual:
                    for i in range(self.n_input_scans):
                        ...
                        tmp_usefuls = [path for path in tmp_residuals if os.path.split(
                            path)[-1][:-4] in tmp_pending_list]
                        exec(
                        ...
                        if i >= 2:
                            exec(
                                f"assert len(self.residual_files_{i-1}[\'{seq}\']) == len(self.residual_files_{i}[\'{seq}\'])")
